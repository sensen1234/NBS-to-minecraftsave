...
class CoordinatePickerDialog(QDialog):
    ...
    def init_ui(self):
        ...
        control_panel.setStyleSheet(
            "QGroupBox { border: none; background-color: #f3f3f3; }"
        )
        ...
        for g_id, data in self.all_groups.items():
            ...
            try:
                gx, gy, _ = int(coords[0]), int(coords[1]), int(coords[2])
            except Exception:
                gx, gy, _ = 0, 64, 0
        ...
        self.target_item = TrackGroupItem(
            self.x,
            self.y,
            self.target_id,
            is_active=True,
            on_move_callback=self.on_point_dragged,
        )
